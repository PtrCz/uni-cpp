...
TEST_CASE("upp::ascii_char from() & from_lossy()", "[upp::ascii_char]")
{
	...
	for (const auto& test : tests)
	{
		...
		std::optional<upp::ascii_char> opt_uint8 = upp::ascii_char::from(value);
		std::optional<upp::ascii_char> opt_char8 = upp::ascii_char::from(static_cast<char8_t>(value));
		std::optional<upp::ascii_char> opt_char	 = upp::ascii_char::from(std::bit_cast<char>(value));

		CHECK(opt_uint8.has_value() == is_valid);
		CHECK(opt_char8.has_value() == is_valid);
		CHECK(opt_char.has_value() == is_valid);

		if (is_valid)
		{
			CHECK(upp::ascii_char::from_lossy(value).value() == value);
			CHECK(upp::ascii_char::from_lossy(static_cast<char8_t>(value)).value() == value);
			CHECK(upp::ascii_char::from_lossy(std::bit_cast<char>(value)).value() == value);
		}
		else
		{
			CHECK(upp::ascii_char::from_lossy(value) == upp::ascii_char::substitute_character());
			CHECK(upp::ascii_char::from_lossy(static_cast<char8_t>(value)) == upp::ascii_char::substitute_character());
			CHECK(upp::ascii_char::from_lossy(std::bit_cast<char>(value)) == upp::ascii_char::substitute_character());
		}

		if (opt_uint8.has_value())
			CHECK(opt_uint8.value().value() == value);

		if (opt_char8.has_value())
			CHECK(opt_char8.value().value() == value);

		if (opt_char.has_value())
			CHECK(opt_char.value().value() == value);
	}
}
...
TEST_CASE("upp::uchar from() & from_lossy()", "[upp::uchar]")
{
	...
	for (const auto& test : tests)
	{
		...
		std::optional<upp::uchar> opt_uint32 = upp::uchar::from(value);
		std::optional<upp::uchar> opt_char32 = upp::uchar::from(static_cast<char32_t>(value));

		CHECK(opt_uint32.has_value() == is_valid);
		CHECK(opt_char32.has_value() == is_valid);

		if (is_valid)
		{
			CHECK(upp::uchar::from_lossy(value).value() == value);
			CHECK(upp::uchar::from_lossy(static_cast<char32_t>(value)).value() == value);
		}
		else
		{
			CHECK(upp::uchar::from_lossy(value) == upp::uchar::replacement_character());
			CHECK(upp::uchar::from_lossy(static_cast<char32_t>(value)) == upp::uchar::replacement_character());
		}

		if (opt_uint32.has_value())
			CHECK(opt_uint32.value().value() == value);

		if (opt_char32.has_value())
			CHECK(opt_char32.value().value() == value);
	}
}
