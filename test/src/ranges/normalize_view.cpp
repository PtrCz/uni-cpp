...
TEST_CASE("normalize_view", "[ranges][normalization]")
{
    auto test_equality = [](auto&& actual_range, auto&& expected_range) {
        ...
        if constexpr (std::ranges::bidirectional_range<decltype(actual_range)>)
    ...
    for (const auto& test_case : test_data)
    {
        ...
        auto test_for_range = [&](auto&& range) {
            test_equality(range | upp::views::normalize_to_nfc, test_nfc);
            ...
            test_equality(range | upp::views::normalize_to_nfd, test_nfd);
            ...
            test_equality(range | upp::views::normalize_to_nfkc, test_nfkc);
            ...
            test_equality(range | upp::views::normalize_to_nfkd, test_nfkd);
