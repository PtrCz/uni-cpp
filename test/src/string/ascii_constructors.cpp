...
TEST_CASE("upp::basic_ascii_string from_ascii()", "[string types]")
{
    ...
    {
        for (const auto& test_case : upp_test::ascii::invalid_test_cases())
        {
            upp_test::string_input_range<char> input_range{std::string_view{test_case.input}};

            const auto result             = upp::ascii_string::from_ascii(test_case.input);
            const auto input_range_result = upp::ascii_string::from_ascii(input_range);
...
TEST_CASE("upp::basic_ascii_string from_ascii_lossy()", "[string types]")
{
    for (const auto& test_case : upp_test::ascii::invalid_test_cases())
    {
        ...
        const auto result             = upp::ascii_string::from_ascii_lossy(test_case.input);
        const auto input_range_result = upp::ascii_string::from_ascii_lossy(input_range);

        CHECK(result.underlying() == test_case.ascii_lossy);
        CHECK(input_range_result.underlying() == test_case.ascii_lossy);
    }
}
...
TEST_CASE("upp::basic_ascii_string from_ascii_unchecked()", "[string types]")
{
    for (const auto& valid_sequence : upp_test::ascii::valid_sequences())
    {
        const auto result = upp::ascii_string::from_ascii_unchecked(valid_sequence);
